...
IC int SetBackTrajectoryCnt(const dReal* p, const dReal* last_pos, Triangle& neg_tri, dxGeom* o1, dxGeom* o2,
		...
		if (neg_depth < dInfinity)
		{
			...
			if (no_last_pos)
			    for (auto& it : pos_tries)
				{
					...
					if (TriContainPoint(
						(dReal*)&V_array[it.T->verts[0]],
						(dReal*)&V_array[it.T->verts[1]],
						(dReal*)&V_array[it.T->verts[2]],
						it.norm, 
						it.side0,
						it.side1, p))
						if (
							negative_tri_set_ignored_by_positive_tri(neg_tri, it, V_array)
						)
						{
							include = false;
							break;
	...
	if (b_neg_depth < dInfinity)
	{
		...
		if (no_last_pos)
			for (auto& it : pos_tries)
			{
				...
				if (
					negative_tri_set_ignored_by_positive_tri(b_neg_tri, it, V_array)
				)
				{
					include = false;
					break;
