...
ALife::ERelationType CAI_Trader::tfGetRelationType(const CEntityAlive* tpEntityAlive) const
{
    ...
    const CInventoryOwner* pSelf = static_cast<const CInventoryOwner*>(this);

    ALife::ERelationType relation = ALife::eRelationTypeDummy;

    if (pOtherIO && !(const_cast<CEntityAlive*>(tpEntityAlive)->cast_base_monster()))
        relation = RelationSystem::Instance().Executor()->RelationType()->GetTotalRelationType(
        const_cast<CInventoryOwner*>(pSelf), const_cast<CInventoryOwner*>(pOtherIO));
