...
bool CScriptGameObject::isInsideRestrictor(CSpaceRestrictor* restrictor) const
{
    if (restrictor)
    {
        Fsphere selfSphere;
        selfSphere.set(object().Position(), object().Radius());

        return restrictor->inside(selfSphere);
    }
    return false;
}

bool CScriptGameObject::IsInRestrictor(LPCSTR restrictorName) const
{    
    auto* obj = Level().Objects.FindObjectByName(restrictorName);    
    if (obj)
    {
        CSpaceRestrictor* zone = smart_cast<CSpaceRestrictor*>(obj);
        return isInsideRestrictor(zone);
    }
    return false;
}

CScriptGameObject* CScriptGameObject::GetCurrentRestrictor() const
{
    for (auto i = 0; i < Level().Objects.ActiveCount(); ++i)
    {
        auto* obj = Level().Objects.IterateActive(i);
        if (obj)
        {
            CSpaceRestrictor* zone = smart_cast<CSpaceRestrictor*>(obj);
            if (zone && isInsideRestrictor(zone))
            {
                return zone->lua_game_object();
            }
        }
    }
    return nullptr;
