    ...
    else
    {
        ...
        if (mn.similar(cur_rect.lt, 2.f) && mx.similar(cur_rect.rb, 2.f))
        {
            ...
            if (auto* pActor = Actor())
            {
                ...
                {
                    switch (RelationSystem::Instance().Executor()->RelationType()->GetTotalRelationType(others_inv_owner, our_inv_owner))
                    {
...
void CBinocularsVision::Load(const shared_str& section)
{
    ...
    m_snd_found.create(pSettings->r_string(section, "found_snd"), SoundType::Effect, sg_SourceType);
