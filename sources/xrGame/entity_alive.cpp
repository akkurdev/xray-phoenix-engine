...
void CEntityAlive::Hit(SHit* pHDS)
{
    ...
    if (g_Alive())
    {
        ...
        {
            RelationSystem::Instance().Controller()->RegisterAction(EA, this, ActionType::Wound, HDS.boneID);
        }
    }
}
...
{
    auto wounds = this->conditions().wounds();
    auto bone = wounds.empty()
        ? -1
        : wounds.back()->GetBoneNum();
    
    RelationSystem::Instance().Controller()->RegisterAction(smart_cast<CEntityAlive*>(who), this, ActionType::Kill, bone);
    inherited::Die(who);
