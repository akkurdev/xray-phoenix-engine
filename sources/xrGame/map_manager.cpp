...
CMapManager::~CMapManager() { delete_data(m_locations); }
...
CMapLocation* CMapManager::AddRelationLocation(CInventoryOwner* pInvOwner)
{
    ...
    relation = RelationSystem::Instance().Executor()->RelationType()->GetTotalRelationType(pInvOwner, pActor);
    shared_str sname = RelationSystem::Instance().Executor()->RelationType()->GetSpotName(relation);
...
Locations& CMapManager::Locations() { return m_locations->registry().objects(); }
