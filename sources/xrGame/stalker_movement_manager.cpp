...
void CStalkerMovementManager::Load(LPCSTR section) { inherited::Load(section); }
...
void CStalkerMovementManager::set_nearest_accessible_position() { set_nearest_accessible_position(object().Position(), object().ai_location().level_vertex_id()); }
...
void CStalkerMovementManager::on_travel_point_change(const u32& previous_travel_point_index) { inherited::on_travel_point_change(previous_travel_point_index); }
...
void CStalkerMovementManager::setup_speed_from_animation(const float& speed) { set_desirable_speed(object().m_fCurSpeed = speed); }
...
void CStalkerMovementManager::force_update(const bool& force_update) { m_force_update = force_update; }
