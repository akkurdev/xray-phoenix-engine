...
bool CRelationMapLocation::Update()
{
    ...
    if (ai().get_alife())
    {
        ...
        CInventoryOwner* iOwner = smart_cast<CInventoryOwner*>(Level().Objects.net_Find(m_pInvOwnerEntityID));
        CInventoryOwner* iActor = smart_cast<CInventoryOwner*>(g_actor);

        m_last_relation = RelationSystem::Instance().Executor()->RelationType()->GetTotalRelationType(iOwner, iActor);
    ...
    else
    {
        ...
        m_last_relation = RelationSystem::Instance().Executor()->RelationType()->GetTotalRelationType(pEnt, pAct);
    ...
    if (bAlive == false)
        ...
        sname = RelationSystem::Instance().Executor()->RelationType()->GetSpotName(m_last_relation);
