...
void CUICharacterInfo::UpdateRelation()
{
    ...
    else
    {
        ...
        CObject* object = Level().Objects.net_Find(m_ownerID);
        CInventoryOwner* iOwner = smart_cast<CInventoryOwner*>(object);
        CInventoryOwner* iActor = smart_cast<CInventoryOwner*>(Actor());

        SetRelation(
            RelationSystem::Instance().Executor()->RelationType()->GetTotalRelationType(iOwner, iActor), 
            RelationSystem::Instance().Executor()->RelationType()->GetTotalRelationAmount(iOwner, iActor));
    }
}
...
void CUICharacterInfo::Update()
{
    ...
    if (hasOwner() && (m_bForceUpdate || (Device.dwFrame % 100 == 0)))
    {
        ...
        auto ownerObject = ai().alife().objects().object((ALife::_OBJECT_ID)m_ownerID, true);
        auto isExists = ai().get_alife() ? ownerObject != nullptr : false;

        CSE_ALifeTraderAbstract* T =isExists ? ch_info_get_from_id(m_ownerID) : NULL;
        if (NULL == T)
        {
            m_ownerID = u16(-1);
            return;
