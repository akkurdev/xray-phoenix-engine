...
using Stalker::Mechanics::Watchers::WatcherSystem;
...
void CALifeStorageManager::save(LPCSTR save_name, bool update_name)
{
    ...
    void* dest_data;
    {
        ...
        spawns().save(stream);
        objects().save(stream);
        RelationSystem::Instance().Controller()->Save(stream);
        WatcherSystem::Instance().Controller()->Save(stream);
...
void CALifeStorageManager::load(void* buffer, const u32& buffer_size, LPCSTR file_name)
{
    ...
    RelationSystem::Instance().Controller()->Load(source);
    WatcherSystem::Instance().Controller()->Load(source);
