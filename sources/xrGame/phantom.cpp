...
void CPhantom::Load(LPCSTR section)
{
    ...
    if (snd_name && snd_name[0])
        m_state_data[stBirth].sound.create(snd_name, SoundType::Effect, sg_SourceType);

    m_state_data[stFly].particles = pSettings->r_string(section, "particles_fly");
    snd_name = pSettings->r_string(section, "sound_fly");
    if (snd_name && snd_name[0])
        m_state_data[stFly].sound.create(snd_name, SoundType::Effect, sg_SourceType);
    ...
    if (snd_name && snd_name[0])
        m_state_data[stContact].sound.create(snd_name, SoundType::Effect, sg_SourceType);

    m_state_data[stShoot].particles = pSettings->r_string(section, "particles_shoot");
    snd_name = pSettings->r_string(section, "sound_shoot");
    if (snd_name && snd_name[0])
        m_state_data[stShoot].sound.create(snd_name, SoundType::Effect, sg_SourceType);
}
