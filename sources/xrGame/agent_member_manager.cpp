...
void CAgentMemberManager::remove(CEntity* member)
{
    ...
    auto I = std::find_if(m_members.begin(), m_members.end(), CMemberPredicate(stalker));
    if (I != m_members.end())
    {
        xr_delete(*I);
        m_members.erase(I);
...
bool CAgentMemberManager::registered_in_combat(const CAI_Stalker* object) const { return (!!(m_combat_mask & mask(object))); }
