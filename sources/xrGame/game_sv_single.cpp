...
void game_sv_Single::OnCreate(u16 id_who)
{
    ...
    if (alife_object->ID_Parent != 0xffff)
    {
        ...
        if (parent)
        {
            CSE_InventoryBoxAbstract* box = smart_cast<CSE_InventoryBoxAbstract*>(parent);
...
BOOL game_sv_Single::OnTouch(u16 eid_who, u16 eid_what, BOOL bForced)
{
    ...
    if (ai().get_alife())
    {
        ...
        if (l_tpALifeInventoryItem && l_tpDynamicObject && ai().alife().graph().level().object(l_tpALifeInventoryItem->base()->ID, true) &&
...
void game_sv_Single::OnDetach(u16 eid_who, u16 eid_what)
{
    if (ai().get_alife())
    {
        ...
        if (ai().alife().objects().object(e_who->ID, true) && !ai().alife().graph().level().object(l_tpALifeInventoryItem->base()->ID, true) &&
