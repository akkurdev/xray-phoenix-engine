...
bool CPHSimpleCharacter::ValidateWalkOnObject()
{
    ...
    // decide to clamb
    if (!m_elevator_state.Active() && b_valide_wall_contact && (m_contact_count > 1) && (m_wall_contact_normal[1] < M_SQRT1_2) &&
...
void CPHSimpleCharacter::doCaptureExist(bool& do_exist) { do_exist = !!m_capture_joint; }
...
void CPHSimpleCharacter::get_spatial_params() { spatialParsFromDGeom((dGeomID)m_space, spatial.sphere.P, AABB, spatial.sphere.R); }
