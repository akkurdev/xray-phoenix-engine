...
int get_community_goodwill(LPCSTR community, int target_id)
{
    CHARACTER_COMMUNITY c;
    c.set(community);
    return RelationSystem::Instance().Executor()->CommunityGoodwill()->Get(c.index(), u16(target_id));
}

void set_community_goodwill(LPCSTR community, int target_id, int value)
{
    CHARACTER_COMMUNITY c;
    c.set(community);
    RelationSystem::Instance().Executor()->CommunityGoodwill()->Set(c.index(), u16(target_id), value);
}

void change_community_goodwill(LPCSTR community, int target_id, int value)
{
    CHARACTER_COMMUNITY c;
    c.set(community);
    RelationSystem::Instance().Executor()->CommunityGoodwill()->Change(c.index(), u16(target_id), value);
}

int get_personal_goodwill(int _who_id, int target_id)
{
    return RelationSystem::Instance().Executor()->PersonalGoodwill()->Get(u16(_who_id), u16(target_id));
}

void set_personal_goodwill(int source_id, int target_id, int value)
{
    RelationSystem::Instance().Executor()->PersonalGoodwill()->Set(u16(source_id), u16(target_id), value);
}

void change_personal_goodwill(int source_id, int target_id, int value)
{
    RelationSystem::Instance().Executor()->PersonalGoodwill()->Change(u16(source_id), u16(target_id), value);
}

int get_reputation(int target_id)
{
    return RelationSystem::Instance().Executor()->Reputation()->Get(u16(target_id));
}

void set_reputation(int target_id, int value)
{
    RelationSystem::Instance().Executor()->Reputation()->Set(u16(target_id), value);
}

void change_reputation(int target_id, int value)
{
    RelationSystem::Instance().Executor()->Reputation()->Change(u16(target_id), value);
}

int get_rank(int target_id)
{
    return RelationSystem::Instance().Executor()->Rank()->Get(u16(target_id));
}

void set_rank(int target_id, int value)
{
    RelationSystem::Instance().Executor()->Rank()->Set(u16(target_id), value);
}

void change_rank(int target_id, int value)
{
    RelationSystem::Instance().Executor()->Rank()->Change(u16(target_id), value);
}
...
{
    module(L, "relation_system")[
        def("get_community_goodwill", &get_community_goodwill),
        def("set_community_goodwill", &set_community_goodwill),
        def("change_community_goodwill", &change_community_goodwill),
        def("get_personal_goodwill", &get_personal_goodwill),
        def("set_personal_goodwill", &set_personal_goodwill),
        def("change_personal_goodwill", &change_personal_goodwill),
        def("get_reputation", &get_reputation),
        def("set_reputation", &set_reputation),
        def("change_reputation", &change_reputation),
        def("get_rank", &get_rank),
        def("set_rank", &set_rank),
        def("change_rank", &change_rank)
    ];
    ...
    module(L, "level")[
        ...
        def("map_change_spot_ser", &map_change_spot_ser),
        ...
        def("actor_ladder_allowed", actor_ladder_allowed),
        def("set_actor_allow_pda", set_actor_allow_pda), 
        ...
        def("demo_record_start", &demo_record_start), 
        def("demo_record_stop", &demo_record_stop),
        def("demo_record_get_position", &demo_record_get_position), 
        def("demo_record_set_position", &demo_record_set_position),
        def("demo_record_get_HPB", &demo_record_get_HPB), 
        def("demo_record_set_HPB", &demo_record_set_HPB),
