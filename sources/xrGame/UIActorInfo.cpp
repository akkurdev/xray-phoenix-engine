...
void CUIActorInfoWnd::FillPointsDetail(const shared_str& id)
{
    ...
    {
        std::sort(section.data.begin(), section.data.end(), [](const auto& a, const auto& b) {
...
void CUIActorInfoWnd::FillReputationDetails(CUIXml* xml, LPCSTR path)
{
    ...
    for (int i = 0; i < cnt; ++i)
    {
        ...
        CHARACTER_GOODWILL gw = RelationSystem::Instance().Executor()->CommunityGoodwill()->Get(comm.index(), Actor()->ID());
