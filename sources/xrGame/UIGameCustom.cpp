...
void CUIGameCustom::OnFrame()
{
    ...
    m_custom_statics.erase(std::remove_if(m_custom_statics.begin(), m_custom_statics.end(), predicate_remove_stat()), m_custom_statics.end());
...
void CUIGameCustom::UpdateStaticPriorityIndex()
{
    if (m_custom_statics_sorted.size() != m_custom_statics.size())
    {
        ...
        std::sort(m_custom_statics_sorted.begin(), m_custom_statics_sorted.end(),
                  [this](auto lhs, auto rhs) { return m_custom_statics[lhs].m_priority < m_custom_statics[rhs].m_priority; });
    }
}
