...
void CSE_ALifeMonsterAbstract::on_unregister()
{
    ...
    if (!ai().alife().is_unloading())
        RelationSystem::Instance().Executor()->RelationType()->ResetById(ID);
    brain().on_unregister();
...
ALife::EMeetActionType CSE_ALifeMonsterAbstract::tfGetActionType(CSE_ALifeSchedulable* tpALifeSchedulable, int iGroupIndex, bool bMutualDetection)
{
    ...
    if (ALife::eCombatTypeMonsterMonster == ai().alife().combat_type()) {
        ...
        return						(ALife::eRelationTypeFriend == ai().alife().relation_type(this,smart_cast<CSE_ALifeMonsterAbstract*>(tpALifeSchedulable)) ?
    ALife::eMeetActionTypeIgnore : ((bMutualDetection || alife().choose_combat_action(iGroupIndex) == ALife::eCombatActionAttack) ? ALife::eMeetActionTypeAttack :
