...
bool CBaseMonster::at_home() { return !m_first_tick_object_not_at_home || (Device.dwTimeGlobal < m_first_tick_object_not_at_home + 4000); }
...
void CBaseMonster::SetTurnAnimation(bool turn_left) { (turn_left) ? anim().SetCurAnim(eAnimStandTurnLeft) : anim().SetCurAnim(eAnimStandTurnRight); }
    ...
    else
    {
        ...
        else
        {
            ...
            case MonsterSound::eMonsterSoundIdle:
                ...
                break;
            case MonsterSound::eMonsterSoundEat: delay = u32(float(db().m_dwEatSndDelay) * _sqrt(float(objects_count))); break;
            case MonsterSound::eMonsterSoundAggressive:
            case MonsterSound::eMonsterSoundPanic: delay = u32(float(db().m_dwAttackSndDelay) * _sqrt(float(objects_count))); break;
            }
...
BOOL CBaseMonster::feel_touch_on_contact(CObject* O) { return (inherited::feel_touch_on_contact(O)); }

BOOL CBaseMonster::feel_touch_contact(CObject* O) { return inherited::feel_touch_contact(O); }
...
float CBaseMonster::get_attack_on_move_attack_radius() { return m_attack_on_move_params.attack_radius; }

float CBaseMonster::get_attack_on_move_update_side_period() { return m_attack_on_move_params.update_side_period; }

float CBaseMonster::get_attack_on_move_prediction_factor() { return m_attack_on_move_params.prediction_factor; }

float CBaseMonster::get_attack_on_move_prepare_radius() { return m_attack_on_move_params.prepare_radius; }

float CBaseMonster::get_attack_on_move_prepare_time() { return m_attack_on_move_params.prepare_time; }
