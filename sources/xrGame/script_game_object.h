...
class CScriptGameObject
{
    ...
    bool isInsideRestrictor(CSpaceRestrictor* restrictor) const;
...
public:
    ...
    /// <summary>
    /// Находится ли объект в рестрикторе с указанным именем
    /// </summary>
    /// <param name="restrictorName">Имя рестриктора</param>
    bool IsInRestrictor(LPCSTR restrictorName) const;

    /// <summary>
    /// Возвращает первый найденный рестриктор, в котором находится объект
    /// </summary>
    CScriptGameObject* GetCurrentRestrictor() const;
